...
Endpoints:
  ...
  POST /api/v1/narrative/generate      — Generate scenario narrative (Nebraska-validated)
  ...
  GET  /api/v1/session/{id}/nebraska   — Nebraska Protocol audit for session
  WS   /ws/session/{id}                — Real-time session WebSocket
  WS   /ws/analytics/{id}             — Real-time analytics WebSocket

Nebraska Protocols:
  Every narrative component generated by this engine must pass the runtime
  contract enforced by the Nebraska Generative System (NGS):
    "State the X. Name the Y. Prove the conversion serves the Law. Otherwise, reject."

  Axis stack applied to narrative generation:
    X-Axis   → Expansion (candidate generation)
    Y-Axis   → Primary Gate (𝕍ᵧ: X→Y under Axiom)
    Y²-Axis  → Clean Conversions (no weasel logic)
    Y³-Axis  → System Coherence (chain composes)
    Z-Axis   → Premise Validity (First Thing carries a deficit)
...
def generate_narrative_snippet(session: dict) -> str:
    """
    Generate a narrative fragment, validated through the Nebraska Protocol Y-axis gate.

    Process:
      1. Build candidate fragments (LLM + fallback pool)
      2. Run through Nebraska X→Y validation under the session's Axiom
      3. Return the first valid fragment; fall back to the strongest candidate
         if all fail the gate (with intervention logged)
    """
...
Output ONLY the narrative text — no introduction, explanation, or markdown formatting."""
    ...
    if OLLAMA_AVAILABLE:
        try:
            ...
            if text:
                ...
                candidate_names.append("llm_primary")
        except Exception as exc:
            print(f"[OLLAMA] Error: {exc}")

    # Always include fallback fragments as additional candidates for Nebraska scoring
    idx = int(intensity * (len(FALLBACK_FRAGMENTS) - 1))
    fallback_idx = min(idx + recursion_level - 1, len(FALLBACK_FRAGMENTS) - 1)
    candidates.append(FALLBACK_FRAGMENTS[fallback_idx])
    candidate_names.append("fallback_primary")

    # Add one more fallback for richer candidate pool
    alt_idx = min(fallback_idx + 1, len(FALLBACK_FRAGMENTS) - 1)
    if alt_idx != fallback_idx:
    ...
    # All candidates failed — axis intervention fires, return strongest candidate
    for comp in components:
        if proto.axis_intervention(comp):
            session["nebraska_intervention_count"] = (
                session.get("nebraska_intervention_count", 0) + 1
            )
...
async def create_experience(req: ExperienceCreateRequest):
    ...
    sessions[exp_id] = {
        ...
        # Nebraska Protocol state
        "nebraska_axiom": FEAR_AXIOMS.get(primary_fear, DEFAULT_AXIOM),
        "nebraska_valid_count": 0,
        "nebraska_rejected_count": 0,
        "nebraska_intervention_count": 0,
        "nebraska_last_qc": None,
    }
...
async def narrative_generate(req: NarrativeGenerateRequest):
    """
    Generate a full scenario narrative, validated through the Nebraska Protocol
    full axis stack:

      Z-meta  →  X-gen  →  Y-gate  →  Y²-check  →  Y³-coherence  →  Newton QC

    The Nebraska Axiom is extracted from the primary fear vector.  Only
    components that pass the X→Y conversion gate under that Axiom are
    included in the returned narrative.
    """
    fear_str = ", ".join(req.fear_vectors) if req.fear_vectors else "existential"
    primary_fear = req.fear_vectors[0] if req.fear_vectors else "existential"

    # ── Nebraska setup ────────────────────────────────────────────────────────
    proto = NebraskaProtocol.from_fear_type(primary_fear)
    axiom = proto.axiom

    # Z-axis: validate premise carries a built-in deficit
    premise = f"{fear_str} horror escape room with {req.story_arc} arc"
    z_valid = proto.z_axis_meta(premise)
    if not z_valid:
        print(f"[NEBRASKA] Z-axis WARN: premise may lack deficit — {premise!r}")
    ...
    # ── Candidate component generation (X-axis) ───────────────────────────────
    # Define candidate narrative components: setup + puzzles + choices
...
Governing Law (Axiom): {axiom}
Fear vectors: {fear_str}.
{constraints_txt}
Story arc: {req.story_arc}. Recursion depth: {req.recursion_depth}.

For each section below, write one or two sentences. Every section must:
- Identify the deficit (X) it addresses
- Deliver a transformation or revelation (Y) that serves the Axiom
...
Keep it dark, immersive, and pedagogically meaningful."""
        try:
            ...
            for label in ("SETUP", "PUZZLE 1", "PUZZLE 2", "PUZZLE 3", "CHOICE A", "CHOICE B"):
                marker = label + ":"
                if marker in raw:
                    start = raw.index(marker) + len(marker)
                    # Find the next label or end of string
                    next_pos = len(raw)
                    for other in ("SETUP:", "PUZZLE 1:", "PUZZLE 2:", "PUZZLE 3:", "CHOICE A:", "CHOICE B:"):
                        if other != marker and other in raw[start:]:
                            pos = raw.index(other, start)
                            next_pos = min(next_pos, pos)
                    section_text = raw[start:next_pos].strip()
                    if section_text:
                        llm_sections[label] = section_text
        except Exception as exc:
            print(f"[OLLAMA] Narrative generate error: {exc}")

    # Build candidate list from LLM sections + fallbacks
    fallback_map = {
        "SETUP": (
            f"You find yourself in a space designed around {fear_str}. "
            "The architecture itself seems to breathe. Something watches from the geometry."
        ),
        "PUZZLE 1": (
            f"Decode the pattern left by those who came before — it is watching you decode it."
        ),
        "PUZZLE 2": "Face the reflection that knows your secrets before you do.",
        "PUZZLE 3": "Choose what to leave behind. The choice has already been made.",
        "CHOICE A": "Confront the source directly — it has been expecting you.",
        "CHOICE B": "Find the hidden exit before it finds you. It knows the map better.",
    }

    for label, fallback in fallback_map.items():
        text = llm_sections.get(label, fallback)
    ...
    print(
        f"[NEBRASKA] narrative_generate | Axiom: {axiom!r} | "
        f"Generated: {len(candidate_texts)} | Valid: {len(survivors)} | "
        f"QC: {'PASS' if qc_report['pass'] else 'FAIL'}"
    )

    # ── Assemble validated narrative ──────────────────────────────────────────
    valid_map = {c.name: c.content for c in survivors}

    setup = valid_map.get("setup", fallback_map["SETUP"])
    puzzles_raw = [
        valid_map.get(f"puzzle_{i+1}", fallback_map[f"PUZZLE {i+1}"])
        for i in range(3)
    ]

    narrative_text = (
        f"SETUP: {setup}\n"
        + "\n".join(f"PUZZLE {i+1}: {p}" for i, p in enumerate(puzzles_raw))
        + f"\nCHOICE A: {valid_map.get('choice_a', fallback_map['CHOICE A'])}"
        + f"\nCHOICE B: {valid_map.get('choice_b', fallback_map['CHOICE B'])}"
    )
            ...
            "key_puzzles": [
                {
                    ...
                    "nebraska_valid": f"puzzle_{i+1}" in valid_map,
                }
...
async def nebraska_audit(session_id: str):
    """
    Nebraska Protocol audit for an active session.

    Returns the session's governing Axiom, component validity counts,
    intervention log, and latest Newton QC checksum result.

    Formula: a/b = story; story = meaning
    Contract: every narrative component must convert X→Y under the Axiom or be excluded.
    """
    if session_id not in sessions:
        raise HTTPException(404, "Session not found")
    ...
    valid_count = session.get("nebraska_valid_count", 0)
    rejected_count = session.get("nebraska_rejected_count", 0)
